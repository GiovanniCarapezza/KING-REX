...
path_input="edgeR_input"
path_output=paste("Imbalance analysis",sep="")
if(!dir.exists(path_output)){dir.create(path_output,recursive = T)}

samples_to_test=c("KM12","LC2AD","KARPAS299","U118MG")

#output file
filter_pval=1E-100
filter_FC=6
out_fusion=paste("fusion","_pv_",filter_pval,"_fc_",filter_FC,".txt",sep="")
write(x = c("Sample","Kinase","FC","PVAL","IN_A","IN_B","OUT_A","OUT_B"),file = paste(path_output,out_fusion,sep="/"),ncolumns = 8,append = F,sep = "\t")
