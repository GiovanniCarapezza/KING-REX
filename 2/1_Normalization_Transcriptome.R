...
#path to raw counts
path_input_data=paste(sep="",path_to_results,"/Results/raw_counts_transcriptome/")
#path reformat rawcount for deseq
path=paste("raw_deseq_transcriptome",sep="")
if(!dir.exists(path)){dir.create(path,recursive = T)}
path_output=paste("normalized_counts_transcriptome",sep="")
...
input=read.table(paste(path_to_results,"/Results/annotations_Transcriptome.txt",sep=""),sep="\t",stringsAsFactors=F,header = T)
...
melt_norm=melt(count_norm_log2)
melt_norm=melt_norm[melt_norm$value>0,]
colnames(melt_norm)[3]="Log2 Norm Count > 0"
colnames(melt_norm)[2]="Samples"
p=ggplot(melt_norm, 
         aes(x=Samples, y=`Log2 Norm Count > 0`)) + coord_cartesian(ylim=c(0,25)) +
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=.5),text = element_text(size=24)) + geom_boxplot()
print(p)
